if getgenv().PandaHub then 
	if game.CoreGui:FindFirstChild("Panda Hub GUI") then
		for i, v in ipairs(game.CoreGui:GetChildren()) do
			if string.find(v.Name, "Panda Hub") then
				v:Destroy()
			end
		end
	end 
end
getgenv().PandaHub = true
--[[share by dio  https://discord.gg/MjGt2bTWBS]]

-- UI bootstrap: wait for LocalPlayer/PlayerGui before accessing TouchGui.
...
local DisableAnimation = PlayerGui and PlayerGui:FindFirstChild("TouchGui")
-- ═══ PANDA HUB COLORS (100% panda theme) ═══
local T1UIColor = {
	...
	["Search Icon Highlight Color"] = Color3.fromRGB(0, 200, 80),
	...
	["Title Text Color"] = Color3.fromRGB(0, 200, 80),
	...
	["Background Image"] = "",
	["Page Selected Color"] = Color3.fromRGB(0, 200, 80),
	["Section Text Color"] = Color3.fromRGB(0, 200, 80),
	["Section Underline Color"] = Color3.fromRGB(0, 200, 80),
	["Toggle Border Color"] = Color3.fromRGB(0, 200, 80),
	...
	["Dropdown Selected Color"] = Color3.fromRGB(0, 200, 80),
	["Dropdown Selected Check Color"] = Color3.fromRGB(0, 200, 80),
	["Textbox Highlight Color"] = Color3.fromRGB(0, 200, 80),
	["Box Highlight Color"] = Color3.fromRGB(0, 200, 80),
	["Slider Line Color"] = Color3.fromRGB(0, 60, 25),
	["Slider Highlight Color"] = Color3.fromRGB(0, 200, 80),
...
Library_Function.Gui.Name = 'Panda Hub GUI'
Library_Function.Gui.ResetOnSpawn = false
Library_Function.Gui.IgnoreGuiInset = true
...
function Library:CreateWindow(Setting)
	...
	uistr.Thickness = 1;
	uistr.Color = Color3.fromRGB(0, 200, 80);


	-- Reveal only after the complete base window hierarchy has been constructed.
	...
	TopStroke.BackgroundColor3 = Color3.fromRGB(0, 200, 80)
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
    TextLabelMain.Text = tostring(TitleNameMain or "Panda Hub - Blox Fruit")
	TextLabelMain.TextColor3 = getgenv().UIColor["GUI Text Color"]
	...
	pageControlStroke.Color = Color3.fromRGB(0, 200, 80)
	...
	function Main_Function:AddTab(PageName)
        ...
        function sectionFunction:AddButton(Setting, Callback)
            ...
            TextColor_1.Parent = RowBG_1
             ...
             ClickArea_1.BackgroundColor3 = Color3.fromRGB(0, 60, 25)
...
function sectionFunction:AddKeyBind(Setting, Callback)
	...
	-- Base UI is complete; only now allow the ScreenGui to become visible.
...
local Window = Library:CreateWindow({
    Title = "Panda Hub - Blox Fruit",
    SubTitle = "Banana Cat Hub",
    SaveFolder = "PandaHub.json",
    Image = "rbxassetid://122444592900332"
})

-- ═══ PANDA HUB TAB LAYOUT (user defined) ═══
...
local SettingsTab     = Window:AddTab("Setting Farm")
local HoldSkillTab    = Window:AddTab("Hold and Select Skill")
local MainTab         = Window:AddTab("Farming")
local StackFarmTab    = Window:AddTab("Stack Farming")
local OtherFarmingTab = Window:AddTab("Farming Other")
local FRDTab          = Window:AddTab("Fruit and Raid,Dungeon")
local SeaEventTab     = Window:AddTab("Sea Event")
local RaceTab         = Window:AddTab("Upgrade Race")
local UpgradeTab      = Window:AddTab("Get and Upgrade Items")
local VolcanoTab      = Window:AddTab("Volcano Event")
local TeleportTab     = Window:AddTab("Tab Wedhook")
local StatsTab        = Window:AddTab("PvP")
local MiscTab         = Window:AddTab("Tab Stats")

-- Internal aliases (keep old TRon variable names pointing to new tabs)
local LocalPlayerTab  = PlayerTab
local HoldAndSkillTab = HoldSkillTab
local MultiFarmTab    = StackFarmTab
local OthersTab       = OtherFarmingTab
local FruitTab        = FRDTab
local RaidTab         = FRDTab
local DungeonTab      = FRDTab
local FarmingHopTab   = UpgradeTab
local DragonDojoTab   = VolcanoTab
local SeaStackTab     = SeaEventTab
local EspTab          = TeleportTab
local SeaSettingsTab  = SettingsTab
local CombatTab       = StatsTab



-- Script Loaded notification (fires after UI is ready)
task.delay(2, function()
	Library:Notify({Title = "🐼 Panda Hub", Content = "Script loaded! Welcome, " .. game.Players.LocalPlayer.Name, Icon = "star", Duration = 5});
end);

InfoTab:AddSection(" 🐼 Panda Hub ");
InfoTab:AddParagraph({
	Title = "Discord",
	Desc = "https://discord.gg/NZm9MXzmzn"
});
InfoTab:AddParagraph({
	Title = "Panda Hub",
	Desc = ""
...
InfoTab:AddButton({
	Title = "Join Panda Hub Discord",
	Desc = "",
	Callback = function()
		setclipboard("https://discord.gg/NZm9MXzmzn");
		Library:Notify({Title = "Panda Hub", Content = "Discord link copied!\ndiscord.gg/NZm9MXzmzn", Icon = "bell", Duration = 5});
	end
});
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Panda Hub", Content = "Item especial encontrado! Auto Chest parado.", Icon = "bell", Duration = 6});
				return;
			end;
			...
			for _, entry in ipairs(chests) do
				...
				if _isSpecialChestItem() then
					_chestTweenActive = false;
					_G.Settings.Farm["Auto Farm Chest Tween"] = false;
					Library:Notify({Title = "Panda Hub", Content = "Item especial encontrado!", Icon = "bell", Duration = 6});
					break;
				end;
			...
			if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
				_G.ChestHopCount = 0;
				Library:Notify({Title = "Panda Hub", Content = "Chest Hop: limite atingido, trocando server...", Icon = "bell", Duration = 4});
				task.wait(2);
				local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
				module:Teleport(game.PlaceId);
			end;
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Panda Hub", Content = "Item especial encontrado! Chest Bypass parado.", Icon = "bell", Duration = 6});
				return;
			end;
		...
		if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
			_G.ChestHopCount = 0;
			Library:Notify({Title = "Panda Hub", Content = "Chest Hop: trocando server...", Icon = "bell", Duration = 4});
			task.wait(2);
			local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
			module:Teleport(game.PlaceId);
		end;
...
local function ShowResetConfirm()
	pcall(function()
		...
		simBtn.MouseButton1Click:Connect(function()
			...
			Library:Notify({Title = "Panda Hub", Content = "Settings resetados! Reexecute o script.", Icon = "bell", Duration = 6});
		end);
...
-- ──── RACE NORMAL (INJECTED FROM BANANA SKIDDER) ────
RaceTab:AddSection("Race Normal");

RaceTab:AddToggle({
	Title = "Auto Get Ghoul",
	Desc = "Kills Cursed Captain to get Ghoul Race",
	Value = false,
	Callback = function(Value)
		_G.GhoulGet = Value
	end
...
RaceTab:AddToggle({
	...
	Desc = "Gets Microchip and kills Order for Cyborg",
	Value = false,
	Callback = function(Value)
		_G.CyborgGet = Value
	end
...
spawn(function()
	while wait(0.5) do
		if _G.CyborgGet then
			pcall(function()
				local replicated = game:GetService("ReplicatedStorage")
				if not replicated:FindFirstChild("Order") and not workspace.Enemies:FindFirstChild("Order") then
				...
				if replicated:FindFirstChild("Order") or workspace.Enemies:FindFirstChild("Order") then
					...
					if v then
						repeat wait() G.Kill(v, _G.CyborgGet)
						until not v.Parent or v.Humanoid.Health <= 0
					else
						game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-6217.2021484375, 28.047645568848, -5053.1357421875)
					end
				end
			end)
		end
...
RaceTab:AddToggle({
	Title = "No Frog",
	Desc = "Removes sky/lighting effects during Race V4 trial",
	Value = false,
	Callback = function(Value)
		_G.NoFrog = Value
	end
...
local function BuyFightStyleFull(styleName)
	...
	if not data then
		Library:Notify({Title = "Panda Hub", Content = "Estilo nao encontrado: " .. tostring(styleName), Icon = "alert-triangle", Duration = 4});
		return;
	end;
	local plr = game.Players.LocalPlayer;
	local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart");
	...
	Library:Notify({Title = "Panda Hub", Content = "Compra enviada: " .. styleName, Icon = "check", Duration = 4});
end;
...
-- ──── INJECTED FROM BANANA SKIDDER ────
ShopTab:AddSection("Race Shop");
ShopTab:AddButton({
	Title = "Buy Ghoul Race",
	Desc = "Requires Ectoplasm x4 (2nd Sea)",
	Callback = function()
...
ShopTab:AddButton({
	...
	Desc = "Requires Microchip from Blackbeard",
	Callback = function()
					...
					if PortalIslands and PortalIslands[_G.SelectIsland] then
						...
						if target then
							...
							if hrp and (hrp.Position - destCF.p).Magnitude < 20 then
								-- Chegou! Para o loop
								_G.BypassTeleportActive = false;
								Library:Notify({Title = "Panda Hub", Content = "Chegou em: " .. _G.SelectIsland, Icon = "map-pin", Duration = 4});
							end;
						end;
					end;
				end);
			...
			if nightTime then
				for _, v in pairs(mystic:GetDescendants()) do
					if v:IsA("MeshPart") and v.Material == Enum.Material.Neon and v.BrickColor == BrickColor.new("Bright blue") then
						...
						task.wait(0.2);
						pcall(function()
							Library:Notify({Title = "Panda Hub", Content = "Quest completa!", Icon = "bell", Duration = 6});
						end);
