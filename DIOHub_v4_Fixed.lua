...
-- ══════════════════════════════════════════════════════════════════════════════
-- DIO HUB V4 COMPLETE CORE
-- Stability layer: character lifecycle, connection registry, safe accessors.
-- Legacy feature implementations are intentionally preserved below.
-- ══════════════════════════════════════════════════════════════════════════════
...
local function DIOGetService(name)
    local cached = DIO_V4.Services[name]
    if cached then return cached end
    local ok, service = pcall(game.GetService, game, name)
    if ok and service then
        DIO_V4.Services[name] = service
        return service
    end
    return nil
end

local function DIOConnect(signal, callback)
    if not signal or type(callback) ~= "function" then return nil end
    local ok, conn = pcall(function()
        return signal:Connect(callback)
    end)
    if ok and conn then
        table.insert(DIO_V4.Connections, conn)
        return conn
    end
end

local function DIODisconnectAll()
    for i = #DIO_V4.Connections, 1, -1 do
        local c = DIO_V4.Connections[i]
        if c then pcall(function() c:Disconnect() end) end
        DIO_V4.Connections[i] = nil
    end
end

local function DIOCharacter()
    local p = DIOGetService("Players")
    local lp = p and p.LocalPlayer
    return lp and lp.Character or nil
end

local function DIORoot()
    local c = DIOCharacter()
    return c and c:FindFirstChild("HumanoidRootPart") or nil
end

local function DIOHumanoid()
    local c = DIOCharacter()
    return c and c:FindFirstChildOfClass("Humanoid") or nil
end

local function DIOAlive()
    local h = DIOHumanoid()
    return h ~= nil and h.Health > 0
end

DIO_V4.GetService = DIOGetService
DIO_V4.Connect = DIOConnect
DIO_V4.DisconnectAll = DIODisconnectAll
DIO_V4.GetCharacter = DIOCharacter
DIO_V4.GetRoot = DIORoot
DIO_V4.GetHumanoid = DIOHumanoid
DIO_V4.Alive = DIOAlive
...
-- ============================================================================
-- DIO HUB v4 - Static Audit / Stability Patch
-- Base: DIOHub_v3_Fixed (1)(1).lua
-- This version preserves the existing feature surface and patches the highest
-- confidence crash/leak/state-management issues without rewriting game logic.
-- ============================================================================
-- ════════════════════════════════════════════════════════════════════════════
...
-- ─────────────────────────────────────────────────────────────────────────────
-- [FIX v3] Key system bypassed — load thẳng không cần nhập key
local _KEY_OK      = true
...
-- ═══════════════════════════════════════════════════════
-- ══════════════════════════════════════════════════════════════════════════════
-- V4 STARTUP COMPATIBILITY LAYER
-- Optional executor APIs are normalized so missing helpers do not abort startup.
-- ══════════════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════════════════
-- END STARTUP COMPATIBILITY LAYER
-- ══════════════════════════════════════════════════════════════════════════════
...
if type(makefolder) == "function" then
    pcall(function()
        if type(isfolder) ~= "function" or not isfolder(_FolderName) then
            makefolder(_FolderName)
        end
    end)
end
...
function LoadSettings()
    if type(isfile) ~= "function" or not isfile(_FullPath) then return false end
...
pcall(function()
    local old = game.CoreGui:FindFirstChild("FPS_Display_80")
...
-- [BananaHub-Fix v2] Fixed: old check waited for PlayerGui.Main.Loading to EXIST,
-- but when the script runs in-game the loading screen is already gone → loop ran forever.
-- Now: just wait for game:IsLoaded() (same as original intent), then wait extra 0.5s as buffer.
...
-- [FIX v3] Replaced unsafe direct index with WaitForChild to prevent crash on load
local _NetFolder = replicated:WaitForChild("Modules"):WaitForChild("Net")
local Remotes = {
    RFJobsRemoteFunction = _NetFolder:WaitForChild("RF/JobsRemoteFunction"),
    RFCraft = _NetFolder:WaitForChild("RF/Craft")
}
...
BringEnemy = function(Mon)
    ...
    if not hrp or not Mon or not Mon.Parent then return end
    ...
    for _, mob in ipairs(workspace.Enemies:GetChildren()) do
        if count >= (_G.MaxBringMobs or 6) then break end

        local hum = mob:FindFirstChildOfClass("Humanoid")
        local root = mob:FindFirstChild("HumanoidRootPart")
        local isRaid = false
        if type(IsRaidMob) == "function" then
            pcall(function() isRaid = IsRaidMob(mob) end)
        end

        if mob ~= Mon and hum and root and hum.Health > 0 and not isRaid then
            local dist = (root.Position - targetPos).Magnitude
            if dist <= (_G.BringRange or 240) and not root:GetAttribute("DIOBringTweening") then
                count += 1
                root:SetAttribute("DIOBringTweening", true)
                root:SetAttribute("DIOBringWalkSpeed", hum.WalkSpeed)
                root:SetAttribute("DIOBringJumpPower", hum.JumpPower)
                ...
                tween.Completed:Once(function()
                    if root and root.Parent then
                        ...
                        if h then
                            h.WalkSpeed = root:GetAttribute("DIOBringWalkSpeed") or 16
                            h.JumpPower = root:GetAttribute("DIOBringJumpPower") or 50
                        end
                        root.CanCollide = true
                        root:SetAttribute("DIOBringTweening", nil)
                        root:SetAttribute("DIOBringWalkSpeed", nil)
                        root:SetAttribute("DIOBringJumpPower", nil)
                    end
                end)
            end
        end
...
-- Optional namecall hook. If the runtime does not expose one of these
-- APIs, skip the hook instead of aborting the entire hub during startup.
if type(getrawmetatable) == "function"
    ...
    if gg and type(old) == "function" then
        pcall(function()
                ...
                if tostring(method) == "FireServer" then
                    if tostring(args[1]) == "RemoteEvent" then
                        if tostring(args[2]) ~= "true" and tostring(args[2]) ~= "false" then
                            if (_G.FarmMastery_G and not SoulGuitar) or (_G.FarmMastery_Dev)
                                or (_G.FarmBlazeEM) or (_G.Prehis_Skills)
                                or (_G.SeaBeast1 or _G.FishBoat or _G.PGB or _G.Leviathan1 or _G.Complete_Trials)
                                or (_G.AimMethod and ABmethod == "Aim Player")
                                or (_G.AimMethod and ABmethod == "Nearest Aim") then
                                args[2] = MousePos
...
DropFruits = function()
  ...
  for a,b2 in pairs((plr.Character and plr.Character:GetChildren()) or {}) do
...
function CheckLegendaryItems()
    local function CheckItem(ITEM_NAME)
        ...
        for i,v in next, ((LocalPlayer.Character and LocalPlayer.Character:GetChildren()) or {}) do
            if v:IsA('Tool') and (v.Name == ITEM_NAME or string.find(v.Name, ITEM_NAME)) then 
                return v 
            end
        end
...
function BypassTP(Target)
    ...
    if CanBypassTeleport(Target) and GetBypassCFrame(Target) then
        ...
        if TargetTP and TargetTP:FindFirstChild("Part") then
            ...
            if lastSpawn and lastSpawn:IsA("Script") then
...
_tp = function(target)
    ...
    -- [FIX v3] Apply Up Y / Same Y / Up Y When Low Health
    local _hum = plr.Character and plr.Character:FindFirstChild("Humanoid")
    if Settings["Up Y"] then
        gg = CFrame.new(gg.X, gg.Y + 30, gg.Z)
    elseif Settings["Up Y When Low Health"] and _hum and _hum.MaxHealth > 0 and (_hum.Health / _hum.MaxHealth) < 0.4 then
        gg = CFrame.new(gg.X, gg.Y + 30, gg.Z)
    elseif Settings["Same Y"] then
        gg = CFrame.new(gg.X, rootPart.Position.Y, gg.Z)
    end

    local distance = (gg.Position - rootPart.Position).Magnitude
    -- [FIX v3] Dung Settings["Tween Speed"] thay vi hardcode 300
    local _tweenSpeed = (Settings["Tween Speed"] and Settings["Tween Speed"] > 0) and Settings["Tween Speed"] or 350
    local tweenInfo = TweenInfo.new(distance / _tweenSpeed, Enum.EasingStyle.Linear)
...
getgenv().AllControls = {}
getgenv().UIToggled = true  -- FIX: was false → UI never auto-showed; now starts visible
...
mainButton.MouseButton1Click:Connect(function()
    if type(Library.ToggleUI) ~= "function" then return end
    Library.ToggleUI()
...
-- [FIX v3] Safe WaitForChild thay vì direct index
local RFSubmarineWorkerSpeak = replicated:WaitForChild("Modules", 10):WaitForChild("Net", 10):WaitForChild("RF/SubmarineWorkerSpeak", 10)  -- [FIX v4]
...
Tabs.Settings:AddSlider({
    Name = "Tween Speed",
    Description = "Toc do di chuyen tween — so cang cao di cang nhanh (default 350)",
    Default = GetSetting("TweenSpeed", 350),
    Min = 50,
    Max = 1000,
    Rounding = 50,
    Callback = function(Value)
...
Tabs.Settings:AddToggle({
    ...
    Description = "Dung spawn point de bypass anti-cheat khi teleport xa",
    Default = GetSetting("BypassTeleport", false),
    Callback = function(Value)
        Settings["Bypass Teleport"] = Value
        _G.SaveData["BypassTeleport"] = Value
        SaveSettings()
    end
})
...
Tabs.Settings:AddToggle({
    Name = "Up Y",
    Description = "Luon teleport len cao hon muc tieu (+30 Y)",
    Default = GetSetting("UpY", false),
    Callback = function(Value)
        Settings["Up Y"] = Value
        _G.SaveData["UpY"] = Value
        SaveSettings()
    end
})

Tabs.Settings:AddToggle({
    Name = "Up Y Khi Mau Thap",
    Description = "Tu dong len cao khi HP duoi 40%",
    Default = GetSetting("UpYLowHP", false),
    Callback = function(Value)
        Settings["Up Y When Low Health"] = Value
        _G.SaveData["UpYLowHP"] = Value
        SaveSettings()
    end
})
...
Tabs.Settings:AddToggle({
    Name = "Same Y",
    Description = "Giu nguyen do cao hien tai khi teleport",
    Default = GetSetting("SameY", false),
    Callback = function(Value)
        Settings["Same Y"] = Value
        _G.SaveData["SameY"] = Value
        SaveSettings()
    end
})

-- Load lai Settings tu file neu AutoSetting bat
if _AutoSetting then
    Settings["Tween Speed"]          = GetSetting("TweenSpeed",     350)
    Settings["Bypass Teleport"]      = GetSetting("BypassTeleport", false)
    Settings["Up Y"]                 = GetSetting("UpY",            false)
    Settings["Up Y When Low Health"] = GetSetting("UpYLowHP",       false)
    Settings["Same Y"]               = GetSetting("SameY",          false)
end
...
local ReplicatedStorage = game:GetService("ReplicatedStorage")
-- [FIX v4] Wrapped in pcall + timeout để không treo/crash khi module chưa load
local FishReplicated, FishingRequest, Config, GetWaterHeight, MaxDistance
pcall(function()
    FishReplicated  = ReplicatedStorage:WaitForChild("FishReplicated", 10)
    if not FishReplicated then return end
    ...
    MaxDistance     = Config and Config.Rod and Config.Rod.MaxLaunchDistance
end)

task.spawn(function()
    while task.wait(0.5) do
        if _G.AutoFishing and FishingRequest then  -- [FIX v4] guard nil FishingRequest
            pcall(function()
...
local RFJobsRemoteFunction3 = ReplicatedStorage3.Modules.Net:WaitForChild("RF/JobsRemoteFunction", 10)  -- [FIX v4] timeout
...
local RFJobToolAbilities4 = ReplicatedStorage4.Modules.Net:WaitForChild("RF/JobToolAbilities", 10)  -- [FIX v4]
...
function DIO_Lifecycle:Track(connection)
    if connection then
...
function DIO_Lifecycle:Destroy()
    ...
    pcall(function()
        ...
        local gui = coreGui:FindFirstChild("FPS_Display_80")
...
-- ══════════════════════════════════════════════════════════════════════════════
-- DIO HUB V4 COMPLETE — FINAL SAFETY LAYER
-- ══════════════════════════════════════════════════════════════════════════════

_G.DIO_V4 = DIO_V4
_G.DIO_V4_Lifecycle = DIO_Lifecycle

-- Keep the cached root synchronized after respawn.
do
    local PlayersService = DIOGetService("Players")
    local lp = PlayersService and PlayersService.LocalPlayer
    if lp then
        DIOConnect(lp.CharacterAdded, function(char)
            pcall(function()
...
-- Safe boolean feature switch helper for future V4 modules.
function DIOFeatureEnabled(name)
...
-- Safe target validation helper for legacy routines.
function DIOValidTarget(model)
...
-- Prevent accidental hard failure from a malformed legacy callback.
function DIOProtectedCall(fn, ...)
