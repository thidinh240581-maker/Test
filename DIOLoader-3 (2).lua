...
loadstring(game:HttpGet("https://pastefy.app/96m6gCwx/raw"))()
do
  ply = game.Players
  plr = ply.LocalPlayer 
...
local redzlib = loadstring(game:HttpGet("https://raw.githubusercontent.com/thidinh240581-maker/Test/refs/heads/main/librarydiofind(Copy).luau"))()
...
local Tabs = {
    ...
    Info = Window:MakeTab({ Title = "Tab Status Server", Icon = "crown" }),
    Main = Window:MakeTab({ Title = "Tab Farming", Icon = "rbxassetid://7733960981" }),
    Hop = Window:MakeTab({ Title = "Tab FPS Boost", Icon = "locate" }),
    ...
    Prehistoric = Window:MakeTab({ Title = "Tab Prehistoric Event", Icon = "tent" }),
    ...
    Travel = Window:MakeTab({ Title = "Tab Teleport Island", Icon = "locate" }),
...
local HopToggle = Tabs.Hop:AddToggle({
    Name = "only this toggle can't use",
    Default = false,
...
Tabs.Settings:AddToggle({
    ...
    Description = "", 
    Default = Settings["Bypass Teleport"], 
    Callback = function(Value)
...
Tabs.Settings:AddToggle({
    ...
    Description = "", 
    Default = Settings["Up Y When Low Health"], 
    Callback = function(Value)
...
RaceCybrog = Tabs.Race:AddToggle({
    Name = "Auto Upgrade Cyborg V1-V3",
    Description = "",
    Default = false,
    Callback = function(Value)
        ...
        if not Value then
            _G.AutoFarmChest   = false
            _G.AutoLawKak      = false
            _G.CyborgSetupDone = false
            _G.CyborgBought    = false  -- reset để tránh skip mua lần sau
        end
    end
})
...
spawn(function()
    while task.wait(Sec) do
        pcall(function()
            if not _G.Auto_Cyborg then return end

            local currentRace = plr.Data.Race.Value

            -- v1
            if currentRace ~= "Cyborg" then
                local hasFist      = plr.Backpack:FindFirstChild("Fist of Darkness") or plr.Character:FindFirstChild("Fist of Darkness")
                local hasCoreBrain = plr.Backpack:FindFirstChild("Core Brain")        or plr.Character:FindFirstChild("Core Brain")
                local hasChip      = plr.Backpack:FindFirstChild("Microchip")         or plr.Character:FindFirstChild("Microchip")
                local lawBoss      = GetConnectionEnemies("Order")

                -- Auto-detect CASE 3: Fist đã gắn rồi (hết trong người)
                -- Dấu hiệu: đang có Chip HOẶC Law đã spawn → Raid đã kích trước đó
                if not _G.CyborgSetupDone and (hasChip or lawBoss) then
                    _G.CyborgSetupDone = true
                end

                -- CASE 4: Có Core Brain 
                if hasCoreBrain and not _G.CyborgBought then
                    _G.AutoFarmChest   = false
                    _G.AutoLawKak      = false
                    _G.CyborgSetupDone = false
                    ...
                    task.wait(2)
                    _G.CyborgBought = true  -- chỉ mua đúng 1 lần

                elseif _G.CyborgSetupDone then
                    if lawBoss then
                        _G.AutoLawKak    = true
                        _G.AutoFarmChest = false
                        repeat
                            task.wait()
                            Attack.Kill(lawBoss, _G.AutoLawKak)
                        until _G.AutoLawKak == false or not lawBoss.Parent or lawBoss.Humanoid.Health <= 0
                        _G.AutoLawKak = false
                        -- Kích Raid để Law respawn lần tiếp
                        fireclickdetector(workspace.Map.CircleIsland.RaidSummon.Button.Main.ClickDetector)
                        task.wait(2)
                    end
                    -- Law chưa spawn → đứng chờ, không làm gì

                -- CASE 2: Có Fist trong người → gắn + mua Chip (nếu chưa có) + kích Raid
                elseif hasFist then
                    _G.AutoFarmChest = false
                    fireclickdetector(workspace.Map.CircleIsland.RaidSummon.Button.Main.ClickDetector)
                    task.wait(1)
                    ...
                    fireclickdetector(workspace.Map.CircleIsland.RaidSummon.Button.Main.ClickDetector)
                    task.wait(2)
                    _G.CyborgSetupDone = true  -- đánh dấu không gắn lại nữa

                -- CASE 1: Chưa có gì → farm rương tìm Fist
                else
                    _G.AutoFarmChest = true
                end
            ...
            -- ── V2 & V3 ─────────────────────────────────────────────────────
                ...
                if alchemistStatus ~= -2 then
                    ...
                    elseif alchemistStatus == 1 then
                        ...
                        elseif not plr.Backpack:FindFirstChild("Flower 3") and not plr.Character:FindFirstChild("Flower 3") then
                            ...
                            if v then
                                ...
                                until plr.Backpack:FindFirstChild("Flower 3") or not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_Cyborg == false
                            else
                                _tp(CFrame.new(980.0985107421875, 121.331298828125, 1287.2093505859375))
                            end
                        end
                    ...
                    local wenStatus = replicated.Remotes.CommF_:InvokeServer("Arowe", "1")
                    if wenStatus == 0 then
                        replicated.Remotes.CommF_:InvokeServer("Arowe", "2")
                    elseif wenStatus == 1 then
                        local hasPhysicalFruit = false
                        for _, v in pairs(plr.Character:GetChildren()) do
                            if v:IsA("Tool") and string.find(v.Name, "Fruit") then
                                hasPhysicalFruit = true
                                break
                            end
                        end
                        if not hasPhysicalFruit then
                            for _, v in pairs(plr.Backpack:GetChildren()) do
                                if v:IsA("Tool") and string.find(v.Name, "Fruit") then
                                    hasPhysicalFruit = true
                        ...
                        if hasPhysicalFruit then
                            replicated.Remotes.CommF_:InvokeServer("Arowe", "3")
                        else
                            for _, fruitName in ipairs(fruit) do
                                local unstored = replicated.Remotes.CommF_:InvokeServer("LoadFruit", fruitName)
                                if unstored then
                                    task.wait(0.5)
                                    for _, v in pairs(plr.Backpack:GetChildren()) do
                                        if v:IsA("Tool") and string.find(v.Name, "Fruit") then
...
RaceGhoul = Tabs.Race:AddToggle({
    Name = "Auto Upgrade Ghoul V1-V3",
    Description = "You need level 1000",
    Default = false,
    Callback = function(Value)
        _G.Auto_Ghoul = Value
    end
...
_G.KilledPlayers = _G.KilledPlayers or {}

spawn(function()
    while task.wait(Sec) do
        pcall(function()
            if not _G.Auto_Ghoul then return end

            local wenl = replicated.Remotes.CommF_:InvokeServer("Wenlocktoad", "1")
            local alch = replicated.Remotes.CommF_:InvokeServer("Alchemist", "1")

            -- ══════════════════════════════════════════════
            -- GHOUL V1 — Wenlocktoad (Ectoplasm + Hellfire Torch)
            -- ══════════════════════════════════════════════
            if wenl ~= -2 then
                if wenl == 0 then
                    replicated.Remotes.CommF_:InvokeServer("Wenlocktoad", "1")

                elseif wenl == 1 then
                    local hasTorch = plr.Backpack:FindFirstChild("Hellfire Torch")
                                  or plr.Character:FindFirstChild("Hellfire Torch")

                    local ectoCount = 0
                    for _, item in pairs(plr.Backpack:GetChildren()) do
                        if item.Name == "Ectoplasm" then ectoCount += 1 end
                    end

                    if hasTorch and ectoCount >= 100 then
                        _G.AutoEctoplasm = false
                        replicated.Remotes.CommF_:InvokeServer("Experimic", "What?")
                        task.wait(0.5)
                        replicated.Remotes.CommF_:InvokeServer("Experimic", "Trade")
                    else
                        _G.AutoEctoplasm = true
                        _tp(CFrame.new(923, 126, 32852))
                        local captain = GetConnectionEnemies("Cursed Captain")
                        if captain then
                            BringEnemy(captain)
                            repeat task.wait()
                                Attack.Kill(captain, _G.Auto_Ghoul)
                            until not captain.Parent or captain.Humanoid.Health <= 0 or not _G.Auto_Ghoul
                        else
                            _tp(CFrame.new(916.928589, 181.092773, 33422))
                        end
                    end
            ...
            -- ══════════════════════════════════════════════
            -- GHOUL V2 — Alchemist (Flower 1, 2, 3)
            -- ══════════════════════════════════════════════
            elseif alch ~= -2 then
                if alch == 0 then
                    replicated.Remotes.CommF_:InvokeServer("Alchemist", "1")
                ...
                elseif alch == 1 then
                    ...
                    elseif not plr.Backpack:FindFirstChild("Flower 3") and not plr.Character:FindFirstChild("Flower 3") then
                        ...
                        if v then
                            repeat task.wait()
                                Attack.Kill(v, _G.Auto_Ghoul)
                            until plr.Backpack:FindFirstChild("Flower 3") or not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_Ghoul == false
                        else
                            _tp(CFrame.new(980.0985107421875, 121.331298828125, 1287.2093505859375))
                        end
                    end
            ...
            -- ══════════════════════════════════════════════
            -- GHOUL V3 — Arowe (Kill 5 players)
            -- ══════════════════════════════════════════════
            else
                local v3 = replicated.Remotes.CommF_:InvokeServer("Arowe", "1")

                if v3 == 0 then
                    _G.KilledPlayers = {}
                    replicated.Remotes.CommF_:InvokeServer("Arowe", "1")

                elseif v3 == 1 then
                    if #_G.KilledPlayers >= 5 then
                        replicated.Remotes.CommF_:InvokeServer("Arowe", "3")
                        _G.KilledPlayers = {}
                    else
                        local target = nil
                        for _, p in pairs(game.Players:GetPlayers()) do
                            if p ~= plr
                                ...
                                and not table.find(_G.KilledPlayers, p.UserId)
                            then
                                local hrp = p.Character:FindFirstChild("HumanoidRootPart")
                                if not hrp then continue end

                                local inSafeZone   = p.Character:FindFirstChildOfClass("ForceField") ~= nil
                                local recentlyDied = p.Character:FindFirstChild("Dying") ~= nil
                                local pvpOk        = not p.Character:FindFirstChild("NoPvP")

                                if not inSafeZone and not recentlyDied and pvpOk then
                                    target = p
                                    break
                                end
                            end
                        ...
                        if target and target.Character then
                            ...
                            if hrp then
                                _tp(hrp.CFrame * CFrame.new(3, 0, 0))
                                repeat task.wait()
                                    Attack.Kill(target.Character, _G.Auto_Ghoul)
                                until not target.Character
                                    or not target.Character.Parent
                                    or target.Character.Humanoid.Health <= 0
                                    or not _G.Auto_Ghoul

                                if not table.find(_G.KilledPlayers, target.UserId) then
                                    table.insert(_G.KilledPlayers, target.UserId)
                                end
                            end
                        end
                    end
        ...
        if not workspace["_WorldOrigin"].Locations:FindFirstChild("Frozen Dimension", true) then                
          ...
          if not myBoat then
            ...
            if plr.Character.Humanoid.Sit == false then
              ...
              repeat wait()
                local targetDestination = CFrame.new(-10000000, 31, 37016.25)
                if CheckEnemiesBoat() or CheckTerrorShark() or CheckPirateGrandBrigade() or  CheckSeaBeast() or CheckHauntedCrew() or CheckFishCrew() or CheckPiranha() or CheckShark() then
                  _tp(CFrame.new(-10000000, 150, 37016.25))
                else
                  _tp(CFrame.new(-10000000, 31, 37016.25))
                end
          ...
          _tp(workspace.Map.FrozenDimension.Center.CFrame*CFrame.new(0,300,0))
        end
      end)
    end
  end
...
Window:Notify({
    Title = "DIO Hub loaders"
    Content = "wait loadlib"
    Image = "rbxassetid://105848560127717"
    Duration = 4
})
Window:Notify({
